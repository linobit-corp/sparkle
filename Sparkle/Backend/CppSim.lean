...
def emitStmt (stmt : Stmt) (typeMap : List (String × HWType))
    (design : Option Design := none)
    (registerOutputNames : List String := []) : StmtParts :=
  match stmt with
  | .assign lhs rhs =>
    ...
    if width > 64 then
      ...
      | .op .mul _ =>
        ...
        , evalTickLocals := [] }
      | .ref srcName =>
        -- Wide reference assign: register-to-wire or wire-to-wire copy
        let sn := sanitizeName lhs
      ...
      | .concat args =>
        ...
        let (lines, _) := revArgs.foldl (fun (acc : List String × Nat) arg =>
          ...
          if argWidth <= 64 then
            ...
            let newLines := if argWidth <= 32 then
              if bitOffset == 0 then
      ...
      | .op .mux muxArgs =>
        ...
        | [cond, thenVal, elseVal] =>
          let condStr := emitExpr typeMap cond
          let thenStr := emitExpr typeMap thenVal
...
def emitModule (m : Module) (design : Option Design := none)
  ...
  if m.isPrimitive then
    ...
    let filteredBody := m.body.filter fun s => match s with
      ...
      | _ => true
    -- Collect register output names (used for dedup and ref-assign ordering below)
    let registerNames := m.body.filterMap fun s => match s with
      | .register output .. => some output
      | _ => none
    -- Collect all StmtParts
    let allParts := filteredBody.map (emitStmt · typeMap design registerNames)
